...
currentDOM = null;
if (!window.hasOwnProperty('attribMap')) attribMap = {};
...
function getInputAttribSafe(node, attrib, def) {
    ...
    if (!node.hasOwnProperty(attrib.toLowerCase())) return def;
    if (!a) return a;
...
function coreDraw(domRoot,cnv, skipSelectors) {
  if (!skipSelectors) runSelectors();
  var jsRoot = normalize(getStructureMain(domRoot, null));
