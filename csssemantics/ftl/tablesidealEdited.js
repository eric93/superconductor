function tablebox_cols_absX ( _ale_arg1,  _ale_arg0) { return _ale_arg0 + _ale_arg1; }
function tablebox_rows_colCount ( _ale_arg0) { return _ale_arg0; }
function tablebox_cols_relY () { return 5; }
function tablebox_cols_availableWidth ( _ale_arg0) { return _ale_arg0; }
...
function tablebox_cols_absY ( _ale_arg0,  _ale_arg1) { return _ale_arg0 + _ale_arg1; }
...
function row_render ( _ale_arg4,  _ale_arg0,  _ale_arg3,  _ale_arg2,  _ale_arg1,  _ale_arg5) { return _ale_arg0 + paintLine(_ale_arg1, _ale_arg2, _ale_arg1 + _ale_arg3, _ale_arg2, _ale_arg4) + paintLine(_ale_arg1 + _ale_arg3, _ale_arg2, _ale_arg1 + _ale_arg3, _ale_arg2 + _ale_arg5, _ale_arg4) + paintLine(_ale_arg1 + _ale_arg3, _ale_arg2 + _ale_arg5, _ale_arg1, _ale_arg2 + _ale_arg5, _ale_arg4) + paintLine(_ale_arg1, _ale_arg2 + _ale_arg5, _ale_arg1, _ale_arg2, _ale_arg4); }
...
function col_childs_relX () { return 5; }
...
function visit_tablebox_2(node) {
  ...
  setAttribSafe(node, "intrinsminwidth_init", ((getAttribSafe(node, "colcount") == 0) ? 10 : 5));
  ...
  logger.log("    last init intrinsminwidth: " + getAttribSafe(node, "intrinsminwidth"));
    ...
    for (var i = 0; i < children.length; i++) {
      var child = children[i]; 
      setAttribSafe(node, "intrinsminwidth", (getAttribSafe(node, "intrinsminwidth") + 5 + getAttribSafe(child, "intrinsminwidth") ));
      logger.log("         step intrinsMinWidth: " + getAttribSafe(node, "intrinsminwidth"));
    }
  })();

  
  setAttribSafe(node, "intrinsprefwidth_init", ((getAttribSafe(node, "colcount") == 0) ? 10 : 5));
  ...
  logger.log("    last init intrinsprefwidth: " + getAttribSafe(node, "intrinsprefwidth"));
    ...
    for (var i = 0; i < children.length; i++) {
      var child = children[i]; 
      setAttribSafe(node, "intrinsprefwidth", (getAttribSafe(node, "intrinsprefwidth") + 5 + getAttribSafe(child, "intrinsprefwidth") ));
...
function visit_row_2(node) {
  ...
  logger.log("    last init computedwidth: " + getAttribSafe(node, "computedwidth"));
    ...
    for (var i = 0; i < children.length; i++) {
      var child = children[i]; 
      setAttribSafe(node, "computedwidth", (max(getAttribSafe(node, "computedwidth"), getAttribSafe(child, "intrinsminwidth"))));
...
function visit_tablebox_4(node) {
  ...
  setAttribSafe(node, "cols_relrightx_init", (0));
  ...
  logger.log("    last init cols_relrightx_last: " + getAttribSafe(node, "cols_relrightx_last"));
    ...
    for (var i = 0; i < children.length; i++) {
      var child = children[i]; 
      setAttribSafe(child, "relrightx", (getAttribSafe(i == 0 ? node : children[i-1], i == 0 ? ("cols_relrightx_init") : ("relrightx")) + 5 + getAttribSafe(child, "computedwidth") ));
...
function visit_tablebox_5(node) {
  ...
  logger.log("    last init rows_rely_last: " + getAttribSafe(node, "rows_rely_last"));
    ...
    for (var i = 0; i < children.length; i++) {
      var child = children[i]; 
      setAttribSafe(child, "relboty", (getAttribSafe(i == 0 ? node : children[i-1], i == 0 ? ("rows_relboty_init") : ("relboty")) + 5 + getAttribSafe(child, "computedheight") ));
...
function visit_row_5(node) {
  ...
  logger.log("    last init intrinsheight: " + getAttribSafe(node, "intrinsheight"));
    ...
    for (var i = 0; i < children.length; i++) {
      var child = children[i]; 
      setAttribSafe(node, "intrinsheight", (max(getAttribSafe(node, "intrinsheight"), getAttribSafe(child, "computedheight") + 10)));
      logger.log("         step intrinsHeight: " + getAttribSafe(node, "intrinsheight"));
    }
  })();
...
function visit_tablebox_6(node) {
  ...
  setAttribSafe(node, "cols_canvas_init", (getAttribSafe(node, "render") ));
...
function visit_row_6(node) {
  ...
  setAttribSafe(node, "render", row_render(getInputAttribSafe(node, "borderc", '#070'), getAttribSafe(node, "canvas"), getAttribSafe(node, "computedwidth"), getAttribSafe(node, "absy"), getAttribSafe(node, "absx"), getAttribSafe(node, "computedheight")));
...
function visit_col_6(node) {
  ...
  setAttribSafe(node, "render", col_render(getInputAttribSafe(node, "borderc", '#770'), getAttribSafe(node, "canvas"), getAttribSafe(node, "computedwidth"), getAttribSafe(node, "absy"), getAttribSafe(node, "absx"), getAttribSafe(node, "computedheight")));
  logger.log("    col_render: " + getAttribSafe(node, "render"));
  logger.log("        borderc: " + getInputAttribSafe(node, "borderc", '#770'));
  logger.log("        canvas: " + getAttribSafe(node, "canvas"));
  logger.log("        computedWidth: " + getAttribSafe(node, "computedwidth"));
  logger.log("        absY: " + getAttribSafe(node, "absy"));
  logger.log("        absX: " + getAttribSafe(node, "absx"));
...
//=========== new footer: reroute some calls
function addCols(table) {
  var numCols = getAttribSafe(node, "colcount");
  var columns = getChildByRefName(table, "columns");
  for (var i = 0; i < numCols; i++) {
    var col = document.createElementByTagName("col");
    setAttributeSafe(col, "refName", "cols");
    columns.appendChild(col);
  }
}
function visit_table_cols(table, visitor) {
  var columns = getChildByRefName(table, "columns");
  for (var i = 0; i < columns.childNodes.length; i++) {
    var node = columns.childNodes[i];
    if (node.nodeType == 1 && node.tagName.toLowerCase() == "col")
      visitor(node);
  }
}
function reroute(visit_i, tag, f) {
...
function ignoreCol (visit_i) { return reroute(visit_i, "col",function(){}); }
function patchedGetChildren(col, children) {
  var res = [];
  var table = col.parentNode.parentNode;
  var rows = getChildren(table, "rows", false);
  var colNum = getAttribSafe(col, "colnum");
  for (var i = 0; i < rows.length; i++) {
    var row = rows[i];
    var cells = getChildren(row, "childs", false);
    for (var j = 0; j < cells.length; j++) {
      var cell = cells[j];
      if (getAttribSafe(cell, "column") == colNum) res.push(cell);
    }
  }
  return res;      
}

function patchGetChildren(v) {
...
function inhTable(visit_i, tablev, colv) {
  return reroute(ignoreCol(visit_i), "tablebox", function (node) { 
      tablev(node);      
      (patchGetChildren(visit_table_cols))(node, colv);
    });
}
function synTable(visit_i, tablev, colv) {
  return reroute(ignoreCol(visit_i), "tablebox", function (node) { 
      tablev(node);
      (patchGetChildren(visit_table_cols))(node, colv);
    });
...
function layout (root) {
  inherit(inhTable(visit_0,visit_tablebox_0,visit_col_0), root);
  inherit(inhTable(visit_1,visit_tablebox_1,visit_col_1), root);
  synthesize(synTable(visit_2,visit_tablebox_2,visit_col_2), root);
  inherit(inhTable(visit_3,visit_tablebox_3,visit_col_3), root);
  inherit(inhTable(visit_4,visit_tablebox_4,visit_col_4), root);
  synthesize(synTable(visit_5,visit_tablebox_5,visit_col_5), root);
  inherit(inhTable(visit_6,visit_tablebox_6,visit_col_6), root);
}
