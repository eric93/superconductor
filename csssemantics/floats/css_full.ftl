...
trait blockPosCont{
    actions{
        ...
        loop posChilds{
            posChilds.posX := (position == "static" || position == "\"static\"") ? posContX : computedX;
            posChilds.posY := (position == "static" || position == "\"static\"") ? posContY : computedY;
            posChilds.posWidth := (position == "static" || position == "\"static\"") ? posContWidth : computedWidth;
...
trait inlinePosCont{
    actions{
        ...
        loop posChilds{
            posChilds.posX := position == ("static" || position == "\"static\"") ? posContX : minX + relX + offsetX;
            posChilds.posY := position == ("static" || position == "\"static\"") ? posContY : minY + relY + offsetY;
            posChilds.posWidth := position == ("static" || position == "\"static\"") ? posContWidth : intrinsPrefWidth;
...
class PositionedBlock(Stacking, fontStyle, countChilds, widthIntrinsics) : Positioned{
    ...
    actions{
        ...
        mt := getTag(marginTop,0,0) == CONST_AUTO() ? 0 : getValue(marginTop,usedFontSize,posWidth);
        mb := getTag(marginBottom,0,0) == CONST_AUTO() ? 0 : getValue(marginBottom,usedFontSize,posWidth);
        ...
        posRelY := (getTag(top) == CONST_AUTO()) ? 0 : getValue(top,usedFontSize,posWidthTmp);
...
class Cell(Stacking, cellWidth, strokeBox, fontStyle, countChilds, widthIntrinsics, blockPosCont) : CellI{
    children{childs : [Block]; posChilds : [Positioned];}
    actions{
        computedX := absX;
        computedY := absY;
...
class Row(fontStyle) : RowI {
  ...
  actions {
    ...
    loop childs {
      ...
      childs.posContX := posX;
      childs.posContY := posY;
      childs.posContWidth := posWidth;
      childs.absY := absY;
    }    
...
class TableBox(inlineblockWidth, strokeBox, widthIntrinsics, blockRelPos) : Block {
  ...
  actions {
    ...
    loop rows {
      ...
      rows.posX := (position == "static" ? posContX : computedX + ml);
      rows.posY := (position == "static" ? posContY : computedY + mt);
      rows.posWidth := (position == "static" ? posContWidth : computedWidth);   
