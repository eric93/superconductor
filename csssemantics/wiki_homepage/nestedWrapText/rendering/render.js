...
function setTagName(node){
    if(node.tagName.toLowerCase() == "contents")
    {
        node.children[0].refName = "child";
        node.tagName = "root";
        return;
    }
    ...
    {
        if(node.tagName.toLowerCase() == "root")
            node.tagName = "wrapbox";
        else
            node.tagName = node.tagName.toLowerCase();

        if(node.refName != "child")
        {
            node.refName = "childs";
        }
    }
...
function getAttribSafe(node, attrib){
    ...
    if(isNaN(n)){
        if(a == "true") return true;
            ...
            //hack to get tables to work
            if(attrib == "margin" && a == "auto")
                return 0;
            return a;
        }
    }
...
function paintParagraph (text, font, fontSize, x, y, maxWidth, allowOverflow, lineHeight, color, lineSpacing) {
  ...
  if(isNaN(x) || x == null || isNaN(y) || y == null) return;
  ctx.save();
  try{
  ctx.translate(x,y);
  ...
  //var lines = ("" + text).trim().split("\n"); see getSumWordW for why newlines are ignored
  var lines = [("" + text).trim()]; 
  ...
  for (var l = 0; l < lines.length; l++) {
    ...
    for (var w = 0; w < words.length; w++) {
      ...
      if (maxWidth && rx && ((rx + space + ww) > maxWidth)) {
        ry += lineHeight + lineSpacing;
        ctx.fillText(words[w].trim(), 0, ry);
        rx = ww;
      } else {
        ctx.fillText((rx ? " " : "") + words[w].trim(), rx, ry);
        rx += (rx ? space : 0) + ww;
      }
    }
...
function getNumberLines(text, _, _, maxWidth) {
    ...
    var returns = [("" + text).trim()]; //Ignore newlines for now (see getSumWordW).
    var lines = [];
    var lastPhrase = "";
    function splitWord() {
    ...
    for (var n = 0; n < returns.length; n++) {
        ...
        for (var i = 0; i < spaces.length; i++) {
            var space = (i == 0 ? "" : " ");
            var measure = ctx.measureText(lastPhrase + space + spaces[i]).width;
